...
class SwinBackbone(SwinTransformer):
    def __init__(self,
        ...
        if pretrained and weight_path is not None:
            print(f"Loading pretrained SwinTransformer weights from {weight_path}")
            state_dict = torch.load(weight_path, map_location='cpu')
            if 'state_dict' in state_dict:
                state_dict = state_dict['state_dict']
            state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items() if k.startswith("backbone.") or k in self.state_dict()}
...
class FTUNetFormer(nn.Module):
    ...
    def __init__(self,
                 ...
                 backbone_weight_path=r'D:/PythonProject/net/weights/swin_tiny_patch4_window7_224.pth'):
        super().__init__()

        self.backbone = SwinBackbone(pretrained=backbone_pretrained,
...
if __name__ == '__main__':
    # 模拟输入：1张图，3通道，512x512
    input_tensor = torch.randn(1, 3, 512, 512)

    # 创建模型（不加载预训练）
    model = FTUNetFormer(backbone_pretrained=True, num_classes=1)
    ...
    print(f"输出尺寸: {output.shape}")  # 应该是 [1, 6, 512, 512]
