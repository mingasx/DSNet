...
class LocalAttentionDecoder(nn.Module):
    ...
    def _enlarge_map(self, pooling_map, feature_map_size):
        ...
        a = pooling_map.view(B, C, 1, -1).repeat(1, 1, basic, 1).view(B, C, -1, 1).repeat(1, 1, 1, basic).view(B, C,
                                                                                                               feature_map_size,
                                                                                                               feature_map_size)
        b = torch.tensor(self._generate_sequence(H, basic)).to(a.device)  # 将 b 移动到 a 的设备
        a = torch.index_select(a, dim=2, index=b)  # a 和 b 现在在相同设备
        return a
...
class DBRANet(nn.Module):
    def __init__(self, pool_size=4):
        ...
        layers = [3, 4, 6, 3]
        top_residual_branch = resnet34(pretrained=False)
